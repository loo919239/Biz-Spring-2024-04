...
public class UserServiceImpl implements UserService {
	 ...
	 * 
	 * 전체 회원 정보 table을 조회하여 최초의 회원가입이라면
	 * ADMIN 과 USER 권한을 부여하고
	 * 두번째 이후의 회원가입 이라면
	 * USER 권한만 부여하는 시스템으로 작동
	 * 
	 * 주의 
	 * @Transactional 을 사용할때 sql 을 실행할때 발생한느 exception 을 
	...
	public int create_user(UserVO userVO) throws DataAccessException {
		...
		if(resultVO == null || resultVO.size() < 1) {
			grantList.add( RoleVO.builder().r_username(userVO.getUsername()).r_role("ROLE_ADMIN").build());
			grantList.add(RoleVO.builder().r_username(userVO.getUsername()).r_role("ROLE_USER").build());
		}else {
			grantList.add(RoleVO.builder().r_username(userVO.getUsername()).r_role("ROLE_USER").build());
		}
