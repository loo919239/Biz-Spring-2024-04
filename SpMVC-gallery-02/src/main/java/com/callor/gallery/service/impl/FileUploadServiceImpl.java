...
public class FileUploadServiceImpl implements FileUploadService {
	...
	public String fileUpload(MultipartFile file) throws Exception {
		...
		/*
		 * context.getRealPath("path") 이 method 는 실행중인 Server 의 Work Folder 를 가리킨다
		 * Server 가 정상적으로 실행되는 동안에는 아무런 문제없이 이 폴더에 파일을 저장하거나, 읽기가 가능하다. 하지만 Tomcat
		 * WorkDirectory Clean 을 실행하면 이 폴더는 삭제 되어 버린다 파일을 업로드 할때 절대 이 폴더에 저장하면 안된다.
		 */
...
//		log.debug("업로드 폴더 {}",folder);
		...
		/*
		 * 같은 이름의 파일을 업로드하여 기존에 업로드된 파일을 변형하는 공격을 막기 위하여 파일이름 앞에 UUID 를 부착하여 저장되는 파일의
		 * 이름을 변경한다
		 */
	...
	public List<ImageVO> filesUpload(MultipartHttpServletRequest files) throws Exception {
		...
		// 업로드된 멀티파일ㅇ르 추출하기 위해서 파일즈.getFiles("이름") 를 사용하는데
		...
		for (MultipartFile f : result) {
			...
			resultImages.add(ImageVO.builder().i_id(UUID.randomUUID().toString()).i_origin_image(f.getOriginalFilename()).i_up_image(resName).build());
		}
		return resultImages;
	}
