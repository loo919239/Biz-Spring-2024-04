...
public class FileServiceImplV1 implements FileService{
	...
	public String fileUp(MultipartFile file) throws Exception {
		...
		if(!path.exists()) {
			 ...
			 * 폴더를 생성하는 method
			 * mkdir() : 한개의 폴더만 생성하기 : app/upload 폴더를 생성할 경우
			 * 		app 폴더가 없으면 오류가 발생
		...
		// 원래의 파일 이름 앞에 uuid 를 부착하여 업로드할 파일 이름 생성
		...
		// DB 에 파일이름을 저장하기 위해서 변경된 파일이름 return 하기
		return fileName;
	}
	...
	public List<ImageVO> filesUp(MultipartHttpServletRequest files) throws Exception {
		...
		for(MultipartFile file : fileList) {
			String resulName = this.fileUp(file);
			String originName = file.getOriginalFilename();
			ImageVO vo = ImageVO.builder()
					.i_up_image(resulName)
					.i_orgin_image(originName)
					.build();
