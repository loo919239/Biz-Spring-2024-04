...
public class FileServiceImpl implements FileService{
	...
	public String filesUp(MultipartFile file) {
		...
		// 문자열 uploadDir 값을 File 객체러 생성 (변환)
	...
	public List<ImagesVO> filesUp(MultipartHttpServletRequest files) {
		...
		for(MultipartFile file : fileList) {
			String resultFileNmae = this.filesUp(file);
			ImagesVO vo = ImagesVO.builder().i_id(UUID.randomUUID().toString()).i_up_image(resultFileNmae).i_seq(index++).i_origin_image(file.getOriginalFilename()).build();
