...
public class GalleryServiceImpl implements GalleryService{
	 ...
	 * 이때 setGalleryVO() method는 GalleryVO 객체를 Call By Reference 로
	 * 받은것과 동일하게 작동된다.
	 */
...
//		gallertyVO.setG_id(g_id);
		...
		if(fileList.size() > 1) {
			...
			String name = fileService.filesUp(fileList.get(0));
	...
	public GalleryVO selectGalleryOne(String gid) {
		...
		List<ImagesVO> images = imageDao.findMyGID(gid);
	...
	public int delete(String id) {
		List<ImagesVO> images = imageDao.findMyGID(id);
