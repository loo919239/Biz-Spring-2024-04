 ...
 * 정상적인 사용자 인지 확인을 하고
 * 사용자 정보와 권한정보가 표함된 객체를 생성하여 Token 을 발행한다.
 */

@Slf4j
@Service("authProvicerImpl")
...
//		String password = (String)authentication.getCredentials();
		...
		if(username == null || username.isBlank() || !username.equals("callor")) {
			/*
			 * 어떠한 method 가 중첩하여 호출이 될때 내부에 깊이 포함된 method 에서
			 * 바깥쪽의 method 에 메세지를 전달하고 싶을때
		...
		UserVO userVO = UserVO.builder().id(username).username("홍길동").email("loo919239@naver.com").build();
		//로그인한 사용자 정보와 권한 리스트를 묶어서 Token 발행하기
