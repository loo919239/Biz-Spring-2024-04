...
public class AuthProviderImpl implements AuthenticationProvider{
	...
	/*
	 * @Autowired
	 * 기본생성자를 통하여 UserDao type 의 userDao 객체를주입받아라 라는 선언
	 * Autowired 를 사용하여 생성자 주입을 하게 되면 
	 * userDao 객체에 final 을 사용할수 없다
	 * 클래스 영역, 필드영역에 선언되는 객체(클래스를 이용하여 선언한 변수들)
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		if(userVO == null) {
			 ...
			 * UsernameNotFoundException을 강제 발생시켜
			 * 코드의 진행을 중단하고 method 를 deep 호출한 곳까지 되돌리기
			 * 결국 Front 까지 메세지를 전달하기 위한 방법
			 */
			throw new UsernameNotFoundException(String.format("USER NAME 확인: %s 는 없음", username));
		}
		
		
		// 로그인 할때 입력한 비번(password)과 
		...
		// 권한 정보(grantList) 를 사용하여 토근(통행증) 발행하기
